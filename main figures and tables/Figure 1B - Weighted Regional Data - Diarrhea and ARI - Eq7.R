# This script is for a figure of Eq7 (diarrhea and ARI)
...
# keep only equation3
eq7_diarrhea <- diarrhea_all_eq_med_CI_weighted %>%
...
# keep only equation3
eq7_ari <- ari_all_eq_med_CI_weighted %>%
  select(matches("Eq7"))

# bind eq3_diarrhea and eq3_ari 
eq7_combined <- cbind(eq7_diarrhea, eq7_ari)


# assuming your dataframe is called df
eq7_combined_long <- eq7_combined %>%
  pivot_longer(
    cols = -region,
    names_to = c("pathogen", ".value"),
    names_pattern = "Eq7_(.*?)_(median|lower|upper)")

eq7_combined_long$pathogen <- factor(eq7_combined_long$pathogen, levels = c("rsv", "pcv", "hib","adeno","rota","noro","ETEC","campy","shigella"),
...
eq7_combined_long <- eq7_combined_long[order(eq7_combined_long$pathogen ),]


q = ggplot(eq7_combined_long, aes(fill=pathogen, x=region, y=median,)) 
...
ggsave(file="Scripts - Aim 3/Figures/Figure - Weighted Regional Data - Diarrhea - ARI - Eq7_25Nov2025.pdf",q,width=22,height=10,dpi=300)
ggsave(file="Scripts - Aim 3/Figures/Figure - Weighted Regional Data - Diarrhea - ARI - Eq7_25Nov2025.png",q,width=22,height=10,dpi=300)

# save eq3_combined_long so that it can be combined with the eq3 data
save(eq7_combined_long, file = "Scripts - Aim 3/Output/Eq7_Figure_output_25Nov2025.Rdata")
