# This script is for a figure of Eq3 - rota - hib - pcv - ACTUAL estimates 
...
# keep only equation3
eq3_hib_pcv_rota <- hib_pcv_rota_all_eq_med_CI_weighted %>%
  select(matches("region|Eq3"))

# make long 
eq3_combined_long <- eq3_hib_pcv_rota %>%
  pivot_longer(
    cols = -region,
    names_to = c("pathogen", ".value"),
    names_pattern = "Eq3_(.*?)_(median|lower|upper)")

eq3_combined_long$pathogen <- factor(eq3_combined_long$pathogen, levels = c("pcv", "hib", "rota"),
...
eq3_combined_long_Rota_Hib_PCV <- eq3_combined_long[order(eq3_combined_long$pathogen ),]


q = ggplot(eq3_combined_long_Rota_Hib_PCV, aes(fill=pathogen, x=region, y=median,)) 
...
ggsave(file="Scripts - Aim 3/Figures/Figure - Weighted Regional Data - Rota - Hib - PCV - Eq3_11Dec2025.pdf",q,width=22,height=10,dpi=300)
ggsave(file="Scripts - Aim 3/Figures/Figure - Weighted Regional Data - Rota - Hib - PCV - Eq3_11Dec2025.png",q,width=22,height=10,dpi=300)

# save eq3_combined_long so that it can be combined with the eq7 data
save(eq3_combined_long_Rota_Hib_PCV, file = "Scripts - Aim 3/Output/Eq3_Figure_output_Rota_Hib_PCV_11Dec2025.Rdata")
